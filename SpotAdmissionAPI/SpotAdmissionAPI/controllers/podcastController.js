...
exports.deletePodcast = asyncHandlerPodcast(async (req, res) => { const p = await Podcast.findById(req.params.id); if (!p) { res.status(404); throw new Error('Podcast not found'); } await p.deleteOne(); res.json({ message: 'Podcast removed' }); });