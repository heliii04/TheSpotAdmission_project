...
exports.getSchools = asyncHandler(async (req, res) => {
    const { location, curriculum, featured, q } = req.query;
    const filter = {};
...
exports.deleteSchool = asyncHandler(async (req, res) => { const school = await School.findById(req.params.id); if (!school) { res.status(404); throw new Error('School not found'); } await school.deleteOne(); res.json({ message: 'School removed' }); });