...
exports.deleteAdmission = asyncHandlerAdmission(async (req, res) => { const admission = await Admission.findById(req.params.id); if (!admission) { res.status(404); throw new Error('Admission not found'); } await admission.remove(); res.json({ message: 'Admission removed' }); });