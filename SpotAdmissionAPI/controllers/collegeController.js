...
exports.deleteCollege = asyncHandlerC(async (req, res) => { const item = await College.findById(req.params.id); if (!item) { res.status(404); throw new Error('College not found'); } await item.remove(); res.json({ message: 'College removed' }); });