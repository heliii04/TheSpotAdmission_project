...
exports.deleteBooking = asyncHandlerCounselling(async (req, res) => { const b = await Counselling.findById(req.params.id); if (!b) { res.status(404); throw new Error('Booking not found'); } await b.remove(); res.json({ message: 'Booking removed' }); });