...
exports.deleteTestimonial = asyncHandlerTest(async (req, res) => { const t = await Testimonial.findById(req.params.id); if (!t) { res.status(404); throw new Error('Testimonial not found'); } await t.remove(); res.json({ message: 'Testimonial removed' }); });