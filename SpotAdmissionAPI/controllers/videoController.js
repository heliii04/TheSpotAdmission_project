...
exports.deleteVideo = asyncHandlerVideo(async (req, res) => { const v = await Video.findById(req.params.id); if (!v) { res.status(404); throw new Error('Video not found'); } await v.remove(); res.json({ message: 'Video removed' }); });