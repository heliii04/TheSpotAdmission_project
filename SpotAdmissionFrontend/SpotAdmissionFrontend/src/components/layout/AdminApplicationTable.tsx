          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                ...
                <th className="p-5 text-xs font-black uppercase tracking-widest text-slate-400">Access Role</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {currentItems.length > 0 ? currentItems.map((app: any) => (
                <tr key={`${app.formType}-${app._id}`} className="hover:bg-indigo-50/30 transition-colors group">
                  ...
                  <td className="p-5">
                    <span className="text-xs font-bold text-slate-600 uppercase">{app.role || 'User'}</span>
                  </td>
                  <td className="p-5">
